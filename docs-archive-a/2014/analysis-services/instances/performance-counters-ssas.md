...
|[MDX](#bkmk_MDX)|Microsoft Analysis Services の MDX 計算に関連する統計。|  
...
|要求の合計|合計接続要求数。  これらは到着した接続要求です。|  
|正常完了数/秒|接続が正常に完了した割合。|  
|合計正常完了数|成功した接続の合計数。|  
|失敗数/秒|接続失敗率。|  
|合計失敗数|失敗した接続試行数の合計。|  
|現在のユーザー セッション数|確立された現在のユーザー セッションの数。|  
...
|クリーナーの現在の価格|現在のメモリ価格 ($/バイト/時刻)。標準は 1,000 です。|  
...
|メモリ使用 KB|クリーナー メモリの価格の計算で使用されるサーバー プロセスのメモリ使用量。  Process\PrivateBytes カウンターの値に、メモリがマップされたデータのサイズを加えた値と等しくなります。xVelocity インメモリ分析エンジン (VertiPaq) によって xVelocity エンジンのメモリの制限を超えてマップされた (割り当てられた) メモリは無視されます。|  
...
|ブロックされているクォータ|他のメモリ クォータが解放されるまでブロックされている現在のクォータ要求の数。|  
...
|VertiPaq のメモリ制限 KB|構成ファイルに指定されているインメモリの制限。|  
...
|短時間解析を行っているジョブ キューの長さ|短時間解析を行っているスレッド プールのキューに登録されているジョブの数。|  
|短時間解析ジョブ率|短時間解析を行っているスレッド プールを介したジョブの比率。|  
|長時間解析を行っているアイドル状態のスレッド|長時間解析を行っているスレッド プール内にあるアイドル状態のスレッドの数。|  
|長時間解析を行っているビジー状態のスレッド|長時間解析を行っているスレッド プール内にあるビジー状態のスレッドの数。|  
|長時間解析を行っているジョブのキューの長さ|長時間解析を行っているスレッド プールのキューに登録されているジョブの数。|  
|長時間解析ジョブ率|長時間解析を行っているスレッド プールを介したジョブの比率。|  
|クエリ プールのアイドル状態のスレッド|クエリ スレッド プール内にあるアイドル状態のスレッドの数。|  
|クエリ プール ビジー状態スレッド数|クエリ スレッド プールのビジー スレッドの数。|  
|クエリ プール ジョブ キューの長さ|クエリ スレッド プールのキューに登録されているジョブの数。|  
|クエリ プール ジョブ キュー率|クエリ スレッド プールを介したジョブの比率。|  
|処理中のプールのアイドル状態の I/O 以外のスレッド|I/O 以外のジョブ専用の、処理中のスレッド プール内にあるアイドル状態のスレッドの数。|  
|処理中のプールのビジー状態の I/O 以外のスレッド|処理中のスレッド プール内で I/O 以外のジョブを実行しているスレッドの数。|  
|処理プール ジョブ キュー長|処理スレッド プールのキュー内の非 I/O ジョブの数。|  
|処理プール ジョブ キュー率|処理中のスレッド プールを介した I/O 以外のジョブの比率。|  
|処理中のプールのアイドル状態の I/O ジョブのスレッド|処理中のスレッド プール内にある I/O ジョブのアイドル状態のスレッドの数。|  
|処理中のプールのビジー状態の I/O ジョブのスレッド|処理中のスレッド プール内で I/O ジョブを実行しているスレッドの数。|  
|処理中のプール I/O ジョブ キューの長さ|処理中のスレッド プールのキューに登録されている I/O ジョブの数。|  
