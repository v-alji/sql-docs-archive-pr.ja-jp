...
-   [列のプロパティの設定を構成するには](#bkmk_config_prop)  
  
##  <a name="column-properties"></a><a name="bkmk_properties"></a>列のプロパティ  
 **Basic**  
  
|プロパティ|既定の設定|説明|  
|--------------|---------------------|-----------------|  
|**列名**||モデルに格納され、レポート クライアントのフィールドの一覧に表示される列の名前。|  
|**データ形式**|インポート時に自動的に決定します。|この列のデータに使用される表示形式を指定します。 データ形式を定義したら、各形式に固有のプロパティを設定できます。 たとえば、 **[通貨]** 形式を選択した場合は、表示される小数点以下の桁数の設定、桁区切り記号の選択、および通貨記号の選択を行います。 このプロパティには、次のオプションがあります。<br /><br /> **全般**<br /><br /> **10 進数**<br /><br /> **整数**<br /><br /> **通貨**<br /><br /> **Percentage**<br /><br /> **[指数]**<br /><br /> 列の値に画像が含まれる場合は、「 **基本画像**」を参照してください。|  
...
|プロパティ|既定の設定|説明|  
|--------------|---------------------|-----------------|  
|[既定の画像]|誤り|行データを表す画像を提供する列 (たとえば、従業員レコードの写真 ID) を指定します。|  
|[既定のラベル]|誤り|行データを表す表示名を提供する列 (たとえば、従業員レコードの従業員名) を指定します。|  
|[画像の URL]/[データ カテゴリ] (SP1)|誤り|サーバー上の画像へのハイパーリンクとして、この列の値を指定します。 (例: http://localhost/images/image1.jpg)。|  
|[一意の行を保持]|誤り|重複している場合でも一意として扱う必要のある値を提供する列を指定します (たとえば、2 人以上の従業員が同じ名前を持つ可能性がある場合の従業員の姓名)。|  
|[行識別子 (ROWID)]|誤り|一意の値のみ含む列を指定します。その列を内部グループ化キーとして使用できます。|  
|集計の方法|Default|この列がフィールドの一覧に追加されたとき、列の計算に集計関数 SUM を適用するレポート クライアント ツールを指定します。 既定の計算を変更するには、ドロップダウン リストから選択します。 このプロパティは、集計が可能な種類の列にのみ適用されます。|  
|[テーブルの詳細の位置]|[既定のフィールド セットがありません]|レポート クライアントでテーブルが見やすくなるように、1 つのテーブルのフィールド セットに追加できる、この列またはメジャーを指定します。|  
  
###  <a name="to-configure-column-property-settings"></a><a name="bkmk_config_prop"></a>列のプロパティ設定を構成するには  
...
## <a name="see-also"></a>参照  
 [SSAS テーブル&#41;&#40;のレポートプロパティの Power View](properties-ssas-tabular.md)   
 [SSAS 表形式&#41;&#40;の列の非表示または固定](hide-or-freeze-columns-ssas-tabular.md)   
