...
---
# <a name="calendar-aware-date-and-time-udt-sample"></a>カレンダー対応の日付と時刻の UDT サンプル
...
4.  C:\MySample で、ファイルを作成 `calendars.txt` し、サンプルコードをファイルにコピーします。  
...
6.  c:\MySample で、ファイル `calendars.ja.txt` を作成し、以下のコードをこのファイルにコピーします。  
  
    -   `; the default calendar for this culture`  
  
    -   `DefaultCalendarID = 3`  
  
7.  c:\MySample で、ファイル `calendars.``zh-CN.txt` を作成し、以下のコードをこのファイルにコピーします。  
  
    -   `; the default calendar for this culture`  
  
    -   `DefaultCalendarID = 8`  
  
8.  c:\MySample で、ファイル `build.com` を作成し、以下のコードをこのファイルにコピーします。  
...
9. ビルドされたファイルをコマンド プロンプトで実行して、サテライト アセンブリをビルドします。  
  
10. 次のコマンドをコマンド プロンプトで実行して、サンプル コードをコンパイルします。  
...
11. [!INCLUDE[tsql](../../includes/tsql-md.md)] インストール コードをファイルにコピーし、`Install.sql` としてサンプル ディレクトリに保存します。  
  
12. サンプルが `C:\MySample\`以外のディレクトリにインストールされている場合は、その場所を示すように、ファイル `Install.sql` を編集します。  
  
13. 次のコマンドを実行して、アセンブリとストアド プロシージャを配置します。  
  
    -   `sqlcmd -E -I -i install.sql`  
  
14. [!INCLUDE[tsql](../../includes/tsql-md.md)]テストコマンドスクリプトをファイルにコピーし、として `test.sql` サンプルディレクトリに保存します。  
  
15. 次のコマンドを使用してテスト スクリプトを実行します。  
  
    -   `sqlcmd -E -I -i test.sql`  
  
16. [!INCLUDE[tsql](../../includes/tsql-md.md)] クリーンアップ スクリプトをファイルにコピーし、`cleanup.sql` としてサンプル ディレクトリに保存します。  
  
17. 次のコマンドを使用してこのスクリプトを実行します。  
...
```  
 ...
 カレンダーの定義に必要なファイル `calendars.txt` を次に示します。  
...
```  
 ...
 次の [!INCLUDE[tsql](../../includes/tsql-md.md)] インストール スクリプト (`Install.sql`) は、アセンブリを配置し、データベースに変換関数を作成します。さらに、サンプル データのテーブルも作成します。  
...
```  
 ...
 次の `test.sql` は、さまざまなカレンダーに対して関数を実行してサンプルをテストします。  
...
```  
 ...
 次の [!INCLUDE[tsql](../../includes/tsql-md.md)] は、アセンブリ、関数、およびテーブルをデータベースから削除します。  
