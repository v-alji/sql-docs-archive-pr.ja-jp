...
3.  c:\MySample で、`Contacts.vb` (Visual Basic サンプル) または `Contacts.cs` (C# サンプル) を作成し、適切な Visual Basic または C# のサンプル コード (下記) をこのファイルにコピーします。  
...
```  
 ...
 次の [!INCLUDE[tsql](../../includes/tsql-md.md)] は、アセンブリ、ストアド プロシージャ、および挿入されたデータをデータベースから削除します。  
