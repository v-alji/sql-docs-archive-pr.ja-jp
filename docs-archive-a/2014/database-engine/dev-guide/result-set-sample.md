---
title: 結果セットのサンプル |Microsoft Docs
ms.custom: ''
ms.date: 06/13/2017
...
ms.locfileid: "87645831"
---
# <a name="result-set-sample"></a><span data-ttu-id="1110c-102">結果セットのサンプル</span><span class="sxs-lookup"><span data-stu-id="1110c-102">Result Set Sample</span></span>
  <span data-ttu-id="1110c-103">新しい接続を開いたりすべての結果をメモリに読み取ったりせずに、クエリの結果を確認しながらコマンドを実行することができると便利な場合があります。</span><span class="sxs-lookup"><span data-stu-id="1110c-103">Sometimes it is useful to be able to execute commands while reading through results of a query without opening a new connection and without reading all the results into memory.</span></span> <span data-ttu-id="1110c-104">ADO.NET 2.0 の複数のアクティブな結果セット (MARS) 機能は、その実行を支援するテクノロジです。</span><span class="sxs-lookup"><span data-stu-id="1110c-104">The Multiple Active Result Set (MARS) feature in ADO.NET 2.0 is a technology that can help you achieve that.</span></span> <span data-ttu-id="1110c-105">現在、MARS は、サーバー側プログラミングに使用されるインプロセス プロバイダーには実装されていません。</span><span class="sxs-lookup"><span data-stu-id="1110c-105">Currently, MARS is not implemented for the in-process provider used for server-side programming.</span></span> <span data-ttu-id="1110c-106">この制限に対処するために、サーバー側カーソルを使用することができます。</span><span class="sxs-lookup"><span data-stu-id="1110c-106">To work around this limitation, you can use server-side cursors.</span></span> <span data-ttu-id="1110c-107">このサンプルでは、サーバー側プログラミングで MARS がサポートされないことに対処するための、サーバー側カーソルの使用方法を示します。</span><span class="sxs-lookup"><span data-stu-id="1110c-107">This sample demonstrates how to use server-side cursors to work around the lack of MARS support for server-side programming.</span></span>  
  
> [!CAUTION]  
>  <span data-ttu-id="1110c-108">サーバー側カーソルの使用はサーバー リソースを多く消費するため、Microsoft SQL Server のクエリ オプティマイザーがクエリのパフォーマンスを向上できなくなる場合があります。</span><span class="sxs-lookup"><span data-stu-id="1110c-108">Using server-side cursors is expensive in terms of server resources and can sometimes prevent the query optimizer in Microsoft SQL Server from enhancing the performance of your queries.</span></span> <span data-ttu-id="1110c-109">そのため、可能な場合は必ずコードを書き直して `JOINs` を使用するようにしてください。</span><span class="sxs-lookup"><span data-stu-id="1110c-109">Therefore, you might consider rewriting your code to use `JOINs` whenever possible.</span></span>  
  
 <span data-ttu-id="1110c-110">このクラスの API は、結果セット内を前後に移動できる点を除いてデータ リーダーと同じであるため、結果セットが開かれている場合でも他のコマンドを接続上で実行することができます。この実装は非常に単純化されているため、このサンプルを簡単に理解することができます。</span><span class="sxs-lookup"><span data-stu-id="1110c-110">The API for this class is similar to a data reader except that you can move forward or backward throughout the result set, and other commands can be issued on the connection while the result set is open.This implementation is highly simplified to make the sample easy to understand.</span></span> <span data-ttu-id="1110c-111">より効率的な実装を行うと、複数の行がフェッチされるため、データベースにかかるフェッチ行あたりの負荷を回避することができます。</span><span class="sxs-lookup"><span data-stu-id="1110c-111">A more efficient implementation would fetch multiple rows to avoid a database turnaround per row fetched.</span></span> <span data-ttu-id="1110c-112">このクラスを使用すると、クエリの結果すべてをデータセットに設定する場合に比べ、メモリの使用量を圧倒的に少なくすることができます。このことは、サーバー側プログラミングで非常に重要です。</span><span class="sxs-lookup"><span data-stu-id="1110c-112">Using this class can have a significantly smaller memory footprint than filling a dataset with all the results of a query, which is very important for server-side programming.</span></span> <span data-ttu-id="1110c-113">また、このサンプルでは、AllowPartiallyTrustedCallers 属性を使用して、ResultSet アセンブリが他のアセンブリから安全に呼び出されるライブラリであることを指定する方法を示します。</span><span class="sxs-lookup"><span data-stu-id="1110c-113">This sample also demonstrates using the "Allow partially trusted callers" attribute to indicate that the Result Set assembly is a library which may be safely called from other assemblies.</span></span> <span data-ttu-id="1110c-114">この方法はやや複雑ですが、UNSAFE 権限を使用して呼び出し側のアセンブリを登録する方法よりはるかに安全性に優れています。</span><span class="sxs-lookup"><span data-stu-id="1110c-114">This approach is a little more complex but much more secure than registering the calling assembly using the unsafe permission.</span></span> <span data-ttu-id="1110c-115">呼び出し側のアセンブリを safe として登録することで、そのアセンブリがサーバー外部のリソースに与える影響が制限されるため、サーバーの整合性が維持されます。</span><span class="sxs-lookup"><span data-stu-id="1110c-115">It is more secure because registering the calling assembly as safe the calling assembly limits affecting resources off the server and prevents damage to the integrity of the server.</span></span>  
  
## <a name="prerequisites"></a><span data-ttu-id="1110c-116">前提条件</span><span class="sxs-lookup"><span data-stu-id="1110c-116">Prerequisites</span></span>  
 <span data-ttu-id="1110c-117">このプロジェクトを作成して実行するには、次のソフトウェアがインストールされている必要があります。</span><span class="sxs-lookup"><span data-stu-id="1110c-117">To create and run this project the following the following software must be installed:</span></span>  
  
-   [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] <span data-ttu-id="1110c-118">または [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express。</span><span class="sxs-lookup"><span data-stu-id="1110c-118">or [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express.</span></span> <span data-ttu-id="1110c-119">[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express は、[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express ドキュメントとサンプルの [Web サイト](https://www.microsoft.com/sql-server/sql-server-editions-express)から無償で入手できます。</span><span class="sxs-lookup"><span data-stu-id="1110c-119">You can obtain [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express free of charge from the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express Documentation and Samples [Web site](https://www.microsoft.com/sql-server/sql-server-editions-express)</span></span>  
  
-   <span data-ttu-id="1110c-120">[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] デベロッパー [Web サイト](https://go.microsoft.com/fwlink/?linkid=62796)から入手できる AdventureWorks データベース。</span><span class="sxs-lookup"><span data-stu-id="1110c-120">The AdventureWorks database that is available at the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Developer [Web site](https://go.microsoft.com/fwlink/?linkid=62796)</span></span>  
  
-   <span data-ttu-id="1110c-121">.NET Framework SDK 2.0 以降または Microsoft Visual Studio 2005 以降。</span><span class="sxs-lookup"><span data-stu-id="1110c-121">.NET Framework SDK 2.0 or later or Microsoft Visual Studio 2005 or later.</span></span> <span data-ttu-id="1110c-122">.NET Framework SDK は無償で入手できます。</span><span class="sxs-lookup"><span data-stu-id="1110c-122">You can obtain .NET Framework SDK free of charge.</span></span>  
  
-   <span data-ttu-id="1110c-123">また、次の条件を満たしている必要があります。</span><span class="sxs-lookup"><span data-stu-id="1110c-123">In addition, the following conditions must be met:</span></span>  
  
-   <span data-ttu-id="1110c-124">使用している [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] インスタンスで CLR 統合が有効になっている必要があります。</span><span class="sxs-lookup"><span data-stu-id="1110c-124">The [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using must have CLR integration enabled.</span></span>  
  
-   <span data-ttu-id="1110c-125">CLR 統合を有効にするには、次の手順に従います。</span><span class="sxs-lookup"><span data-stu-id="1110c-125">In order to enable CLR integration, perform the following steps:</span></span>  
  
    #### <a name="enabling-clr-integration"></a><span data-ttu-id="1110c-126">CLR 統合の有効化</span><span class="sxs-lookup"><span data-stu-id="1110c-126">Enabling CLR Integration</span></span>  
  
    -   <span data-ttu-id="1110c-127">以下の [!INCLUDE[tsql](../../includes/tsql-md.md)] コマンドを実行します。</span><span class="sxs-lookup"><span data-stu-id="1110c-127">Execute the following [!INCLUDE[tsql](../../includes/tsql-md.md)] commands:</span></span>  
    ...
    > [!NOTE]  
    >  <span data-ttu-id="1110c-128">CLR を有効にするには、 `ALTER SETTINGS` サーバーレベルの権限が必要です。この権限は、 `sysadmin` 固定サーバーロールおよびのメンバーによって暗黙的に保持されてい `serveradmin` ます。</span><span class="sxs-lookup"><span data-stu-id="1110c-128">To enable CLR, you must have `ALTER SETTINGS` server level permission, which is implicitly held by members of the `sysadmin` and `serveradmin` fixed server roles.</span></span>  
  
-   <span data-ttu-id="1110c-129">使用している [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] インスタンスに AdventureWorks データベースがインストールされている必要があります。</span><span class="sxs-lookup"><span data-stu-id="1110c-129">The AdventureWorks database must be installed on the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using.</span></span>  
  
-   <span data-ttu-id="1110c-130">使用している [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] インスタンスの管理者でない場合は、インストールを完了するために **CreateAssembly** 権限が管理者から付与されている必要があります。</span><span class="sxs-lookup"><span data-stu-id="1110c-130">If you are not an administrator for the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using, you must have an administrator grant you **CreateAssembly**  permission to complete the installation.</span></span>  
  
## <a name="building-the-sample"></a><span data-ttu-id="1110c-131">サンプルのビルド</span><span class="sxs-lookup"><span data-stu-id="1110c-131">Building the Sample</span></span>  
  
#### <a name="create-and-run-the-sample-by-using-the-following-instructions"></a><span data-ttu-id="1110c-132">次の手順に従ってサンプルを作成し、実行します。</span><span class="sxs-lookup"><span data-stu-id="1110c-132">Create and run the sample by using the following instructions:</span></span>  
  
1.  <span data-ttu-id="1110c-133">Visual Studio または .NET Framework のコマンド プロンプトを開きます。</span><span class="sxs-lookup"><span data-stu-id="1110c-133">Open a Visual Studio or .NET Framework command prompt.</span></span>  
  
2.  <span data-ttu-id="1110c-134">必要な場合は、サンプル用のディレクトリを作成します。</span><span class="sxs-lookup"><span data-stu-id="1110c-134">If necessary, create a directory for your sample.</span></span> <span data-ttu-id="1110c-135">この例では C:\MySample を使用します。</span><span class="sxs-lookup"><span data-stu-id="1110c-135">For this example, we will use C:\MySample.</span></span>  
  
3.  <span data-ttu-id="1110c-136">コマンド プロンプトで次のコードを実行し、厳密な名前のキー ファイルを作成します。</span><span class="sxs-lookup"><span data-stu-id="1110c-136">Create a strong  name key file by executing the following at the command prompt.</span></span>  
  
## <a name="sample-code"></a><span data-ttu-id="1110c-137">サンプル コード</span><span class="sxs-lookup"><span data-stu-id="1110c-137">Sample Code</span></span>  
 <span data-ttu-id="1110c-138">このサンプルのコード リストを次に示します。</span><span class="sxs-lookup"><span data-stu-id="1110c-138">The following are the code listings for this sample.</span></span>  
  
 <span data-ttu-id="1110c-139">次に示すのは、ライブラリ `ResultSet.` のコードです。</span><span class="sxs-lookup"><span data-stu-id="1110c-139">This is the code for the Library `ResultSet.`</span></span>  
  
 <span data-ttu-id="1110c-140">C#</span><span class="sxs-lookup"><span data-stu-id="1110c-140">C#</span></span>  
...
```  
 ...
 <span data-ttu-id="1110c-141">次に示すのは、ライブラリ `ResultSet.` をテストするためのコードです。</span><span class="sxs-lookup"><span data-stu-id="1110c-141">This is the code to test the library `ResultSet.`</span></span>  
  
 <span data-ttu-id="1110c-142">C#</span><span class="sxs-lookup"><span data-stu-id="1110c-142">C#</span></span>  
...
```  
 ...
 <span data-ttu-id="1110c-143">次の [!INCLUDE[tsql](../../includes/tsql-md.md)] インストール スクリプト (`Install.sql`) は、アセンブリを配置し、ストアド プロシージャを作成します。</span><span class="sxs-lookup"><span data-stu-id="1110c-143">This is the [!INCLUDE[tsql](../../includes/tsql-md.md)] installation script (`Install.sql`), which deploys the assemblies and creates the stored procedures.</span></span>  
...
```  
 ...
 <span data-ttu-id="1110c-144">次の [!INCLUDE[tsql](../../includes/tsql-md.md)] テスト スクリプト (`test.sql`) は、サンプルをテストします。</span><span class="sxs-lookup"><span data-stu-id="1110c-144">This is the [!INCLUDE[tsql](../../includes/tsql-md.md)] test script (`test.sql`), which tests the sample.</span></span>  
...
```  
 ...
 <span data-ttu-id="1110c-145">次の [!INCLUDE[tsql](../../includes/tsql-md.md)] は、データベースからアセンブリとストアド プロシージャを削除します。</span><span class="sxs-lookup"><span data-stu-id="1110c-145">The following [!INCLUDE[tsql](../../includes/tsql-md.md)] removes the assemblies and stored procedures from the database.</span></span>  
...
## <a name="see-also"></a><span data-ttu-id="1110c-146">参照</span><span class="sxs-lookup"><span data-stu-id="1110c-146">See Also</span></span>  
 [<span data-ttu-id="1110c-147">CLR &#40;共通言語ランタイム&#41; 統合の使用シナリオと例</span><span class="sxs-lookup"><span data-stu-id="1110c-147">Usage Scenarios and Examples for Common Language Runtime &#40;CLR&#41; Integration</span></span>](../../../2014/database-engine/dev-guide/usage-scenarios-and-examples-for-common-language-runtime-clr-integration.md)  
