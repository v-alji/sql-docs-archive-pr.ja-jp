...
> [!CAUTION]  
>  サーバー側カーソルの使用はサーバー リソースを多く消費するため、Microsoft SQL Server のクエリ オプティマイザーがクエリのパフォーマンスを向上できなくなる場合があります。 そのため、可能な場合は必ずコードを書き直して `JOINs` を使用するようにしてください。  
...
3.  コマンド プロンプトで次のコードを実行し、厳密な名前のキー ファイルを作成します。  
  
## <a name="sample-code"></a>サンプル コード  
 このサンプルのコード リストを次に示します。  
  
 次に示すのは、ライブラリ `ResultSet.` のコードです。  
...
```  
 ...
 次に示すのは、ライブラリ `ResultSet.` をテストするためのコードです。  
...
```  
 ...
 次の [!INCLUDE[tsql](../../includes/tsql-md.md)] インストール スクリプト (`Install.sql`) は、アセンブリを配置し、ストアド プロシージャを作成します。  
...
```  
 ...
 次の [!INCLUDE[tsql](../../includes/tsql-md.md)] テスト スクリプト (`test.sql`) は、サンプルをテストします。  
