...
> [!NOTE]  
>  Excel 変換 `CommandTimeout` 先のプロパティは、 **Excel 変換先エディター**では使用できませんが、**詳細エディター**を使用して設定できます。 また、特定の高速読み込みオプションは、**詳細エディター**でのみ使用できます。 これらのプロパティの詳細については、「 [Excel Custom Properties](data-flow/excel-custom-properties.md)」の「Excel 変換先」を参照してください。  
