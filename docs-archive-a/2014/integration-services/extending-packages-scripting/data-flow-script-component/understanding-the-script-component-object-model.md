...
-   入力行をループするときに追加の処理を行う必要がある場合にのみ、 **\<inputbuffer>_ProcessInput** をオーバーライドします。 (たとえば、 `EndOfRowset` すべての行が処理された後に、他のアクションを実行するためにをテストする必要があります)。行の処理を実行するには、 ** \<inputbuffer> _ProcessInputRow**を呼び出します。
