...
|**ObjectType**|**int**|イベントに関係するオブジェクトの種類を表す値。 この列に返される値は、 **sys.objects** カタログ ビューの **type** 列の対応する値と、 [ObjectType トレース イベント列](objecttype-trace-event-column.md)に示された値の組み合わせになります。 たとえば、8277-U が返された場合、オブジェクトの種類はユーザー定義テーブルです。|28|はい|  
