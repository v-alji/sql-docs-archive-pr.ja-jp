...
|**ObjectType**|**int**|イベントに関係するオブジェクトの種類を表す値。 この列に返される値は、 **sys.objects** カタログ ビューの **type** 列の対応する値と、 [ObjectType トレース イベント列](objecttype-trace-event-column.md)に示された値の組み合わせになります。|28|はい|  
