...
# <a name="sprecompile-event-class"></a>SP:Recompile イベント クラス
 ...
 ステートメント レベルの再コンパイルを追跡する方法としては、SQL:StmtRecompile イベント クラスの使用をお勧めします。 SP:Recompile イベント クラスの使用が非推奨となりました。 詳細については、「 [SQL:StmtRecompile イベント クラス](sql-stmtrecompile-event-class.md)」を参照してください。  
