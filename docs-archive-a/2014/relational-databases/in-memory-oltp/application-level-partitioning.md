...
ms.locfileid: "87632468"
---
# <a name="application-level-partitioning"></a><span data-ttu-id="2b3d5-102">アプリケーション レベルのパーティション分割</span><span class="sxs-lookup"><span data-stu-id="2b3d5-102">Application-Level Partitioning</span></span>
  <span data-ttu-id="2b3d5-103">このサンプルでは、特定の日付の前か後どちらであるかによって、メモリ最適化テーブルかディスク ベース テーブルのいずれかにデータが保存される、アプリケーション レベルのパーティション分割を示します。</span><span class="sxs-lookup"><span data-stu-id="2b3d5-103">This sample demonstrates application-level partitioning where data is stored in either a memory-optimized table or a disk-based table depending on whether the order falls before or after a specific date.</span></span> <span data-ttu-id="2b3d5-104">*ホットな日付*と同じかそれ以降のすべての注文は、メモリ最適化テーブルにあり、*ホットデータ*の前のすべての注文がディスクベーステーブルに含まれます。</span><span class="sxs-lookup"><span data-stu-id="2b3d5-104">All orders newer or equal to the *hotDate* are in the memory-optimized table and all orders before the *hotDate* are in the disk-based table.</span></span> <span data-ttu-id="2b3d5-105">多くの同時トランザクションが含まれる極端な OLTP ワークロードを前提とします。</span><span class="sxs-lookup"><span data-stu-id="2b3d5-105">Assume an extreme OLTP workload with a lot of concurrent transactions.</span></span> <span data-ttu-id="2b3d5-106">複数の同時実行トランザクションが *hotDate*を変更しようとしている場合でも、このビジネス ルール (メモリ最適化テーブルの最近の注文) を適用する必要があります。</span><span class="sxs-lookup"><span data-stu-id="2b3d5-106">This business rule (recent orders in a memory-optimized table) must be enforced even if several concurrent transactions are attempting to change the *hotDate*.</span></span>  
  
 <span data-ttu-id="2b3d5-107">このサンプルでは、ディスクベーステーブルに[パーティションテーブル](../partitions/partitioned-tables-and-indexes.md)を使用せず、3番目のテーブルを使用して2つのテーブル間の明示的な分割点を追跡します。</span><span class="sxs-lookup"><span data-stu-id="2b3d5-107">This sample does not use [Partitioned Tables](../partitions/partitioned-tables-and-indexes.md) for the disk-based table but does track an explicit split point between the two tables using a third table.</span></span> <span data-ttu-id="2b3d5-108">この分岐点を使用して、新しく挿入されるデータが日付に基づいて必ず適切なテーブルに挿入されることを保証できます。</span><span class="sxs-lookup"><span data-stu-id="2b3d5-108">The split point can be used to ensure that newly inserted data is always inserted into the appropriate table based on the date.</span></span> <span data-ttu-id="2b3d5-109">また、どこでデータを検索するかを決定することもできます。</span><span class="sxs-lookup"><span data-stu-id="2b3d5-109">It could also be used to determine where to look for data.</span></span> <span data-ttu-id="2b3d5-110">遅れて到着するデータも、引き続き適切なテーブルに配置されます。</span><span class="sxs-lookup"><span data-stu-id="2b3d5-110">Late arriving data still goes into the appropriate table.</span></span>  
  
 <span data-ttu-id="2b3d5-111">パーティションテーブルを使用する関連のサンプルについては、「[メモリ最適化テーブルをパーティション分割するためのアプリケーションパターン](memory-optimized-tables.md)」を参照してください。</span><span class="sxs-lookup"><span data-stu-id="2b3d5-111">For a related sample that uses partitioned tables, see [Application Pattern for Partitioning Memory-Optimized Tables](memory-optimized-tables.md).</span></span>  
  
## <a name="code-listing"></a><span data-ttu-id="2b3d5-112">コード リスト</span><span class="sxs-lookup"><span data-stu-id="2b3d5-112">Code Listing</span></span>  
...
## <a name="see-also"></a><span data-ttu-id="2b3d5-113">参照</span><span class="sxs-lookup"><span data-stu-id="2b3d5-113">See Also</span></span>  
 <span data-ttu-id="2b3d5-114">[インメモリ OLTP &#40;インメモリ最適化&#41;](in-memory-oltp-in-memory-optimization.md) </span><span class="sxs-lookup"><span data-stu-id="2b3d5-114">[In-Memory OLTP &#40;In-Memory Optimization&#41;](in-memory-oltp-in-memory-optimization.md) </span></span>  
 [<span data-ttu-id="2b3d5-115">インメモリ OLTP のコード サンプル</span><span class="sxs-lookup"><span data-stu-id="2b3d5-115">In-Memory OLTP Code Samples</span></span>](in-memory-oltp-code-samples.md)  
