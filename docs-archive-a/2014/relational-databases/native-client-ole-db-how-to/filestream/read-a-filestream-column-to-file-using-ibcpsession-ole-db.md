...
ms.locfileid: "87713153"
---
# <a name="read-a-filestream-column-to-file-using-ibcpsession-ole-db"></a><span data-ttu-id="ccdad-102">IBCPSession を使用した FILESTREAM 列のファイルへの読み取り (OLE DB)</span><span class="sxs-lookup"><span data-stu-id="ccdad-102">Read a FILESTREAM Column to File Using IBCPSession (OLE DB)</span></span>
  <span data-ttu-id="ccdad-103">このサンプルでは、IBCPSession インターフェイスを使用して filestream 列をファイルに読み取り、フォーマット ファイルを作成します。</span><span class="sxs-lookup"><span data-stu-id="ccdad-103">This sample reads a filestream column to a file using the IBCPSession interface and writes a format file.</span></span>  
  
 <span data-ttu-id="ccdad-104">Filestream 機能の詳細については、「 [Filestream Support &#40;OLE DB&#41;](../../native-client/ole-db/filestream-support-ole-db.md)」を参照してください。</span><span class="sxs-lookup"><span data-stu-id="ccdad-104">For more information on the filestream feature, see [FILESTREAM Support &#40;OLE DB&#41;](../../native-client/ole-db/filestream-support-ole-db.md).</span></span>  
  
## <a name="example"></a><span data-ttu-id="ccdad-105">例</span><span class="sxs-lookup"><span data-stu-id="ccdad-105">Example</span></span>  
 <span data-ttu-id="ccdad-106">INCLUDE 環境変数に、sqlncli を含むディレクトリが含まれていることを確認します。</span><span class="sxs-lookup"><span data-stu-id="ccdad-106">Make sure your INCLUDE environment variable includes the directory that contains sqlncli.h.</span></span>  
  
 <span data-ttu-id="ccdad-107">次のいずれかのサンプルを使用して、このサンプルで読み取るテーブルを作成します。</span><span class="sxs-lookup"><span data-stu-id="ccdad-107">Use one of the following samples to create the table that this sample reads from:</span></span>  
  
-   [<span data-ttu-id="ccdad-108">ICommandText パラメーターにバインドされた ISequentialStream を使用したデータの FILESTREAM 列への送信 (OLE DB)</span><span class="sxs-lookup"><span data-stu-id="ccdad-108">Send Data to a FILESTREAM Column Using ISequentialStream Bound to ICommandText Parameter &#40;OLE DB&#41;</span></span>](send-data-to-filestream-isequentialstream-bound-to-icommandtext.md)  
  
-   [<span data-ttu-id="ccdad-109">IRowsetFastUpload を使用した FILESTREAM 列へのデータの送信 (OLE DB)</span><span class="sxs-lookup"><span data-stu-id="ccdad-109">Send Data to a FILESTREAM Column Using IRowsetFastUpload &#40;OLE DB&#41;</span></span>](send-data-to-a-filestream-column-using-irowsetfastupload-ole-db.md)  
  
 <span data-ttu-id="ccdad-110">1 つ目のコード リストをコピーし、ISSHelper.h という名前のファイルに貼り付けます。</span><span class="sxs-lookup"><span data-stu-id="ccdad-110">Copy the first code listing and paste it into a file called ISSHelper.h.</span></span>  
  
 <span data-ttu-id="ccdad-111">2 つ目のコード リストをコピーし、ISSHelper.cpp という名前のファイルに貼り付けます。</span><span class="sxs-lookup"><span data-stu-id="ccdad-111">Copy the second code listing and paste it into a file called ISSHelper.cpp.</span></span>  
  
 <span data-ttu-id="ccdad-112">3 つ目のコード リストをコピーし、IBCPSession.cpp という名前のファイルに貼り付けます。</span><span class="sxs-lookup"><span data-stu-id="ccdad-112">Copy the third code listing and paste it into a file called IBCPSession.cpp.</span></span>  
  
 <span data-ttu-id="ccdad-113">IBCPSession.cpp、ISSHelper.cpp、ole32.lib、および oleaut32.lib をコンパイルします。</span><span class="sxs-lookup"><span data-stu-id="ccdad-113">Compile IBCPSession.cpp, ISSHelper.cpp, ole32.lib, and oleaut32.lib.</span></span>  
  
 <span data-ttu-id="ccdad-114">このサンプルを実行するには、サーバー名 (または server\instance_name) と、作成されたフォーマット ファイルの名前を渡す必要があります。</span><span class="sxs-lookup"><span data-stu-id="ccdad-114">When this sample is run, you must pass a server name (or server\instance_name) and a name for the format file it will create.</span></span>  
