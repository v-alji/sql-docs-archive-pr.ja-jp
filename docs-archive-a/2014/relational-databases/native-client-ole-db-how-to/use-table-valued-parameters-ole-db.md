...
ms.locfileid: "87739846"
---
# <a name="use-table-valued-parameters-ole-db"></a><span data-ttu-id="4deab-102">テーブル値パラメーターの使用 (OLE DB)</span><span class="sxs-lookup"><span data-stu-id="4deab-102">Use Table-Valued Parameters (OLE DB)</span></span>
  <span data-ttu-id="4deab-103">このサンプルは、[!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)] 以降で動作します。</span><span class="sxs-lookup"><span data-stu-id="4deab-103">This sample works with [!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)] or later.</span></span> <span data-ttu-id="4deab-104">このサンプルでは、次の処理を実行します。</span><span class="sxs-lookup"><span data-stu-id="4deab-104">This sample does the following:</span></span>  
  
-   <span data-ttu-id="4deab-105">IOpenRowset::OpenRowset による動的探索を使用して、テーブル値パラメーターを作成します。</span><span class="sxs-lookup"><span data-stu-id="4deab-105">Creates table-valued parameters by using dynamic discovery though IOpenRowset::OpenRowset.</span></span>  
  
-   <span data-ttu-id="4deab-106">EmployeesRowset クラスのプル モデルを使用してテーブル値パラメーター行を送信します。</span><span class="sxs-lookup"><span data-stu-id="4deab-106">Sends table-valued parameter rows by using the pull model in the EmployeesRowset class.</span></span> <span data-ttu-id="4deab-107">プル モデルでは、コンシューマーが要求時にプロバイダーにデータを提供します。</span><span class="sxs-lookup"><span data-stu-id="4deab-107">In the pull model, the consumer provides data on demand to the provider.</span></span>  
  
-   <span data-ttu-id="4deab-108">CPhotograph クラスでテーブル値パラメーターの一部として BLOB を送信します。</span><span class="sxs-lookup"><span data-stu-id="4deab-108">Sends BLOBs as part of a table-valued parameter in the CPhotograph class.</span></span>  
  
-   <span data-ttu-id="4deab-109">ISSCommandWithParameters を使ったカスタム パラメーター プロパティを使用します。</span><span class="sxs-lookup"><span data-stu-id="4deab-109">Uses custom parameter properties using ISSCommandWithParameters.</span></span>  
  
-   <span data-ttu-id="4deab-110">SQLNCLI11 エラーのエラー処理を表示します。</span><span class="sxs-lookup"><span data-stu-id="4deab-110">Shows error handling for SQLNCLI11 errors.</span></span>  
  
 <span data-ttu-id="4deab-111">テーブル値パラメーターの詳細については、「[テーブル値パラメーター &#40;SQL Server Native Client&#41;](../native-client/features/table-valued-parameters-sql-server-native-client.md)」を参照してください。</span><span class="sxs-lookup"><span data-stu-id="4deab-111">For more information about table-valued parameters, see [Table-Valued Parameters &#40;SQL Server Native Client&#41;](../native-client/features/table-valued-parameters-sql-server-native-client.md).</span></span>  
  
## <a name="example"></a><span data-ttu-id="4deab-112">例</span><span class="sxs-lookup"><span data-stu-id="4deab-112">Example</span></span>  
 <span data-ttu-id="4deab-113">最初の ([!INCLUDE[tsql](../../includes/tsql-md.md)]) コード リストは、このサンプルで使用するデータベースを作成します。</span><span class="sxs-lookup"><span data-stu-id="4deab-113">The first ([!INCLUDE[tsql](../../includes/tsql-md.md)]) code listing creates the database used by the sample.</span></span>  
  
 <span data-ttu-id="4deab-114">2 つ目のコード リストを stdafx.h という名前のファイルに含めます。</span><span class="sxs-lookup"><span data-stu-id="4deab-114">Put the second code listing into a file called stdafx.h.</span></span>  
  
 <span data-ttu-id="4deab-115">3 つ目のコード リストを OLEDBUtils.hpp という名前のファイルに含めます。</span><span class="sxs-lookup"><span data-stu-id="4deab-115">Put the third code listing into a file called OLEDBUtils.hpp.</span></span>  
  
 <span data-ttu-id="4deab-116">ole32.lib と oleaut32.lib を使用して 4 つ目の (C++) コード リストをコンパイルし、実行します。</span><span class="sxs-lookup"><span data-stu-id="4deab-116">Compile with ole32.lib oleaut32.lib and execute the fourth (C++) code listing.</span></span> <span data-ttu-id="4deab-117">このアプリケーションは、コンピューターの既定の [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] インスタンスに接続します。</span><span class="sxs-lookup"><span data-stu-id="4deab-117">This application connects to your computer's default [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance.</span></span> <span data-ttu-id="4deab-118">一部の Windows オペレーティング システムでは、(localhost) または (local) を実際の [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] インスタンスの名前に変更する必要があります。</span><span class="sxs-lookup"><span data-stu-id="4deab-118">On some Windows operating systems, you will need to change (localhost) or (local) to the name of your [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance.</span></span> <span data-ttu-id="4deab-119">名前付きインスタンスに接続するには、接続文字列を L"(local)" から L"(local)\\\name" に変更します。ここで、name は名前付きインスタンスです。</span><span class="sxs-lookup"><span data-stu-id="4deab-119">To connect to a named instance, change the connection string from L"(local)" to L"(local)\\\name" , where name is the named instance.</span></span> <span data-ttu-id="4deab-120">[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express は、既定で名前付きインスタンスとしてインストールされます。</span><span class="sxs-lookup"><span data-stu-id="4deab-120">By default, [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express installs to a named instance.</span></span> <span data-ttu-id="4deab-121">INCLUDE 環境変数に、sqlncli を含むディレクトリが含まれていることを確認します。</span><span class="sxs-lookup"><span data-stu-id="4deab-121">Make sure your INCLUDE environment variable includes the directory that contains sqlncli.h.</span></span>  
  
 <span data-ttu-id="4deab-122">5 つ目の ([!INCLUDE[tsql](../../includes/tsql-md.md)]) コード リストは、このサンプルで使用したデータベースを削除します。</span><span class="sxs-lookup"><span data-stu-id="4deab-122">The fifth ([!INCLUDE[tsql](../../includes/tsql-md.md)]) code listing creates the database used by the sample.</span></span>  
