...
ms.locfileid: "87644465"
---
# <a name="ibcpsessionbcpdone-ole-db"></a><span data-ttu-id="3eb4c-102">IBCPSession::BCPDone (OLE DB)</span><span class="sxs-lookup"><span data-stu-id="3eb4c-102">IBCPSession::BCPDone (OLE DB)</span></span>
  <span data-ttu-id="3eb4c-103">[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] に送信される残りの行をコミットします。</span><span class="sxs-lookup"><span data-stu-id="3eb4c-103">Commits the remaining rows to be sent to [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)].</span></span>  
  
## <a name="syntax"></a><span data-ttu-id="3eb4c-104">構文</span><span class="sxs-lookup"><span data-stu-id="3eb4c-104">Syntax</span></span>  
...
## <a name="remarks"></a><span data-ttu-id="3eb4c-105">解説</span><span class="sxs-lookup"><span data-stu-id="3eb4c-105">Remarks</span></span>  
 <span data-ttu-id="3eb4c-106">**BCPDone** メソッドを呼び出すと、それ以後は [IBCPSession](ibcpsession-ole-db.md) インターフェイスの他の操作を呼び出すことはできません。</span><span class="sxs-lookup"><span data-stu-id="3eb4c-106">No other operation can be called on the [IBCPSession](ibcpsession-ole-db.md) interface after calling the **BCPDone** method.</span></span> <span data-ttu-id="3eb4c-107">ただし、一括コピー操作を新しく開始する [IBCPSession::BCPInit](ibcpsession-bcpinit-ole-db.md) メソッドだけは呼び出すことができます。</span><span class="sxs-lookup"><span data-stu-id="3eb4c-107">The only possibility is to call the [IBCPSession::BCPInit](ibcpsession-bcpinit-ole-db.md) method to initiate a new bulk copy operation.</span></span> <span data-ttu-id="3eb4c-108">これは [IRowsetFastLoad::Commit](irowsetfastload-commit-ole-db.md) メソッドを呼び出した後の状態と同様です。</span><span class="sxs-lookup"><span data-stu-id="3eb4c-108">This is similar to calling the [IRowsetFastLoad::Commit](irowsetfastload-commit-ole-db.md) method.</span></span>  
  
## <a name="return-code-values"></a><span data-ttu-id="3eb4c-109">リターン コードの値</span><span class="sxs-lookup"><span data-stu-id="3eb4c-109">Return Code Values</span></span>  
 <span data-ttu-id="3eb4c-110">S_OK</span><span class="sxs-lookup"><span data-stu-id="3eb4c-110">S_OK</span></span>  
 <span data-ttu-id="3eb4c-111">メソッドが成功しました。</span><span class="sxs-lookup"><span data-stu-id="3eb4c-111">The method succeeded.</span></span>  
  
 <span data-ttu-id="3eb4c-112">E_UNEXPECTED</span><span class="sxs-lookup"><span data-stu-id="3eb4c-112">E_UNEXPECTED</span></span>  
 <span data-ttu-id="3eb4c-113">メソッドの呼び出しが予期されませんでした。</span><span class="sxs-lookup"><span data-stu-id="3eb4c-113">The call to the method was unexpected.</span></span> <span data-ttu-id="3eb4c-114">たとえば、このメソッドを呼び出す前に、**BCPInit** メソッドが呼び出されなかった場合などです。</span><span class="sxs-lookup"><span data-stu-id="3eb4c-114">For example, the **BCPInit** method was not called before calling this method.</span></span>  
  
## <a name="example"></a><span data-ttu-id="3eb4c-115">例</span><span class="sxs-lookup"><span data-stu-id="3eb4c-115">Example</span></span>  
 <span data-ttu-id="3eb4c-116">このサンプルでは、**IBCPSession** インターフェイスの使用方法を示します。</span><span class="sxs-lookup"><span data-stu-id="3eb4c-116">This sample shows how to use the **IBCPSession** interface.</span></span>  
  
 <span data-ttu-id="3eb4c-117">このサンプルを実行する前に、次の [!INCLUDE[tsql](../../includes/tsql-md.md)] を実行する必要があります。</span><span class="sxs-lookup"><span data-stu-id="3eb4c-117">The following [!INCLUDE[tsql](../../includes/tsql-md.md)] must be executed before running this sample:</span></span>  
...
```  
 ...
 <span data-ttu-id="3eb4c-118">このサンプルでは、master データベースに接続します。</span><span class="sxs-lookup"><span data-stu-id="3eb4c-118">The sample connects to the master database.</span></span>  
  
 <span data-ttu-id="3eb4c-119">サンプルの実行時に、outfile.dat がプロジェクト ディレクトリに作成されます。</span><span class="sxs-lookup"><span data-stu-id="3eb4c-119">When the sample runs, it creates outfile.dat in the project directory.</span></span> <span data-ttu-id="3eb4c-120">outfile.dat には、ネイティブ (バイナリ) モードでテーブルからコピーされるデータが格納されます。</span><span class="sxs-lookup"><span data-stu-id="3eb4c-120">The outfile.dat contains the data copied from the table in native (binary) mode.</span></span>  
  
 <span data-ttu-id="3eb4c-121">BCP を使用する次のコマンドで、このデータをテーブルに追加し直すことができます。</span><span class="sxs-lookup"><span data-stu-id="3eb4c-121">You could use BCP to add this data back into the table with the following command:</span></span>  
  
 <span data-ttu-id="3eb4c-122">**bcp master..fltest in outfile.dat -n -T -S** *サーバー*</span><span class="sxs-lookup"><span data-stu-id="3eb4c-122">**bcp master..fltest in outfile.dat -n -T -S** *server*</span></span>  
  
 <span data-ttu-id="3eb4c-123">このサンプルをコンパイルする際には、sqlncli11.lib を指定する必要があります。</span><span class="sxs-lookup"><span data-stu-id="3eb4c-123">You will need to specify sqlncli11.lib when compiling this sample.</span></span>  
...
## <a name="see-also"></a><span data-ttu-id="3eb4c-124">参照</span><span class="sxs-lookup"><span data-stu-id="3eb4c-124">See Also</span></span>  
 <span data-ttu-id="3eb4c-125">[IBCPSession &#40;OLE DB&#41;](ibcpsession-ole-db.md) </span><span class="sxs-lookup"><span data-stu-id="3eb4c-125">[IBCPSession &#40;OLE DB&#41;](ibcpsession-ole-db.md) </span></span>  
 [<span data-ttu-id="3eb4c-126">一括コピー操作の実行</span><span class="sxs-lookup"><span data-stu-id="3eb4c-126">Performing Bulk Copy Operations</span></span>](../native-client/features/performing-bulk-copy-operations.md)  
