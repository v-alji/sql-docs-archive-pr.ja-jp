...
#### <a name="cascading-referential-integrity"></a>連鎖参照整合性
 ...
 親テーブルの対応する行が更新または削除されたときに、外部キーを構成するすべての値が NULL に設定されている場合は、NULL に設定されます。 この制約を実行するには、外部キー列が NULL 値を使用できる必要があります。 INSTEAD OF UPDATE トリガーが設定されているテーブルには指定できません。
