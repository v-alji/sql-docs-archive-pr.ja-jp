 ...
 **パス**  
 ネットワーク接続経由でアクセス可能な共有フォルダーを指定します。 フォルダーがアクセス可能であることを確認するには、[スタート] メニューの [ファイル名を指定して**実行**] をクリックし、 \\ \\<computername \> \\<sharedfoldername の形式でフォルダーパスを入力し \> ます。  
 ...
 **ユーザー名**  
 共有フォルダーにファイルを追加するアクセス許可を持つドメインユーザーアカウントを、<username の形式で入力します \<domain> \\ \> 。  
 ...
 **毎日**  
 選択した日の指定した時刻に実行されるスケジュールを定義します。 曜日は、毎日、すべての *\<day>* 曜日、および *\<number>* 毎日のように指定できます。 いずれかのオプションを選択すると、それ以外のオプションは表示されていても無効になります。  
