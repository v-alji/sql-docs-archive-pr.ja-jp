...
# <a name="removing-a-rendering-extension"></a>表示拡張機能の削除
  表示拡張機能を削除するには [!INCLUDE[ssRSnoversion](../../../includes/ssrsnoversion-md.md)] 、 `Extension` **%ProgramFiles%\Microsoft SQL Server \ MSRS10_50 にある rsreportserver.config ファイルから表示拡張機能の要素を削除するだけです。 \<Instance Name>\ Reporting Services\ReportServer**フォルダー。 レポートサーバーだけでなくレポートデザイナーのエントリを作成した場合は、 `Extension` [Rsreportdesigner 構成ファイル](../../report-server/rsreportdesigner-configuration-file.md)からも要素を削除します。 構成情報を削除した後は、表示拡張機能をコンポーネントに使用できません。  
