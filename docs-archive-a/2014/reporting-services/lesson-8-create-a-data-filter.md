...
5.  [ **=** **演算子**] ボックスの一覧で、等号 () 演算子をクリックします。  
  
6.  [**値**] フィールドの横にある式ボタンをクリックし、[**カテゴリ**] 領域の [**パラメーター** ] をクリックして、[ `productid` **値**] 領域内をダブルクリックします。 **[式の設定: 値]** フィールドに、 **=Parameters!productid.Value**のような式が表示されます。  
...
2.  Integer 型のパラメーターを受け取り、オブジェクトを返す新しい関数を作成し、次のようにし `productid` `datatable` ます。  
...
-   パラメーター値を取得し、その DataTable を子レポートにバインドするイベントハンドラーを追加し `PurchaseOrdeDetail` ます。  
...
5.  イベント ハンドラーを完成させます。 このイベント ハンドラーには、次の機能を含めます。  
    ...
    5.  DataTable を `PurchaseOrderDetail` レポートの対応するデータソースにバインドします。  
