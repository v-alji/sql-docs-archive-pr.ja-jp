...
-   既定のフィールド区切り記号は、コンマ (,) です。  
    ...
    > [!NOTE]  
    >  フィールド区切り記号を任意の文字 (タブなど) に変更するには、デバイス情報設定を変更します。 詳細については、「 [CSV Device Information Settings](../csv-device-information-settings.md)」を参照してください。  
  
-   レコードの区切り記号は、キャリッジリターンとラインフィード ( \<cr> \<lf> ) です。  
