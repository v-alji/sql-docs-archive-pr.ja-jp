 ...
 *result_expression*  
 ( `Variant` ) データセット内の行に対して評価される式*source_expression*  = *destination_expression*、は取得する値を指定します。 たとえば、「 `=Fields!PhoneNumber.Value` 」のように入力します。  
...
## <a name="example"></a>例  
 ...
 次の式は、テキストボックスの値プロパティとして使用されている場合、を使用して `Join` リストを作成します。  
