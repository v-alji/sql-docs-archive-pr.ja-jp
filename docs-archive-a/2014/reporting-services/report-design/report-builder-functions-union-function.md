...
## <a name="return"></a>戻り値  
 `SqlGeometry`式の型に基づいて、またはのいずれかの空間オブジェクトを返し `SqlGeography` ます。 および空間データ型の詳細について `SqlGeometry` `SqlGeography` は、「[空間データ型の概要](../../relational-databases/spatial/spatial-data-types-overview.md)」を参照してください。  
