...
|**問い合わせ**|電子メール メッセージの `Cc` 行に表示される電子メール アドレス。 複数の電子メール アドレスを指定するときは、それぞれをセミコロンで区切ります。 省略可能。|  
...
|**優先順位**|電子メール メッセージを送信する優先順位。 有効な値は `LOW`、`NORMAL`、`HIGH` です。 既定値は `NORMAL` です。|  
|**件名**|電子メール メッセージの件名のテキスト。|  
|**コメント**|電子メール メッセージの本文のテキスト。|  
