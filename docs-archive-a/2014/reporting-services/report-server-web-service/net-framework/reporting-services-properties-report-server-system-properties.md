...
|EnableIntegratedSecurity|統合セキュリティをレポート データ ソース接続でサポートするかどうかを決定します。 既定では、 `True`です。 有効な値は次のとおりです。<br /><br /> `True` = 統合セキュリティが有効になります。<br /><br /> `False` = 統合セキュリティが有効になりません。 統合セキュリティを使用するように構成されているレポート データ ソースは実行されません。|  
