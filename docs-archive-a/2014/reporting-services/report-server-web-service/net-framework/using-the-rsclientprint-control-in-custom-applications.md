...
-   コントロールを \<OBJECT> 指定するには、タグを使用します。  
...
|カルチャ|Int32|RW|ブラウザーのロケール|ロケール ID (LCID) を指定します。 この値によりユーザー入力の測定単位が決まります。 たとえば、ユーザーがを入力した場合、 `3` 言語がフランス語の場合、値はミリメートル単位で測定されます (米国)。 有効な値は、1028、1031、1033、1036、1040、1041、1042、2052、3082 です。|  
...
|認証|Boolean|RW|誤り|セッション外印刷の接続を開始するためにコントロールからレポート サーバーに GET コマンドを発行するかどうかを指定します。|  
...
|引数|I/O|Type|説明|  
|--------------|----------|----------|-----------------|  
|ServerPath|場所|String|レポートサーバーの仮想ディレクトリ (たとえば、) を指定し https://adventure-works/reportserver) ます。|  
