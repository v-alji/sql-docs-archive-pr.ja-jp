...
7.  [**パス**] ボックスに、レポートを配信する既存のフォルダーへの汎用名前付け規則 (UNC) パス (たとえば、 \\ \\<servername \> \\<myreports) を入力し \> ます。 パスの先頭には円記号 (\) を 2 つ含めます。 末尾には円記号を指定しないでください。  
...
9. [**ユーザー名**] ボックスと [**パスワード**] ボックスに、 *\<domain>* \\ ユーザー名の形式を使用して、ファイル共有へのアクセスに必要な資格情報を指定し *\<user name>* ます。  
