...
## <a name="reports-with-custom-modules"></a>カスタム モジュールを含むレポート  
 [!INCLUDE[msCoName](../includes/msconame-md.md)] [!INCLUDE[vbprvb](../includes/vbprvb-md.md)] モジュール内にカスタムコードが含まれている場合は、変換されません。 インポート処理中にレポートデザイナーがコードを検出すると、警告が生成され、[**タスク一覧**] ウィンドウに表示されます。  
