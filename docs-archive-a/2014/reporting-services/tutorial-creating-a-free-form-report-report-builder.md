...
2.  ボックスに「 `Name` **listdatasource** 」と入力します。

3.  **[レポートに埋め込まれた接続を使用する]** をクリックします。

4.  接続の種類が Microsoft SQL Server であることを確認し、[**接続文字列**] ボックスに「 **Data \<servername> Source =** 」と入力します。

     \<servername>たとえば、Report001 などは、SQL Server データベースエンジンのインスタンスがインストールされているコンピューターを指定します。 レポート データは SQL Server のデータベースから抽出されるのではないので、データベース名を含める必要はありません。 指定したサーバー上の既定のデータベースを使用して、クエリが解析されます。
...
2.  ボックスに「 `Name` Listdataset」と入力し**ます。**
...
3.  で `Name` 、既定の名前を「 **Salesinformationbyterritory**」に置き換えます。
...
3.  **プロパティ**ペインで、 `Parent` プロパティに四角形の名前が含まれていることを確認します。
