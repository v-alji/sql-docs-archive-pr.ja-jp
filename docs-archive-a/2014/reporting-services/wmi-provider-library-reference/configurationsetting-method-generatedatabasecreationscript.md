...
> [!IMPORTANT]  
>  以降では [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)] 、 *issharepointmode*はサポートされて = `True` いません。 sharepoint モードでは、 [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] は sharepoint 共有サービスであり、WMI プロバイダーによって制御されないためです。 このパラメーターは常に `False` に設定してください。  
