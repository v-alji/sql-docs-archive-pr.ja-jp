...
1.  [テスト**セットの作成**] ページで、**テスト用データの割合**を既定値のままにし `30` ます。  
  
2.  **[テストデータセット内のケースの最大数**] に、「」と入力 `1000` します。  
...
1.  [**ウィザードの完了**] ページの [**マイニング構造名**] に「」と入力 `Targeted Mailing` します。  
  
2.  [**マイニングモデル名**] に「」と入力 `TM_Decision_Tree` します。  
